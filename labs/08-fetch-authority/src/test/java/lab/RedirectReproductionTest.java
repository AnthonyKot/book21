...
class RedirectReproductionTest extends FetchHttp {
    ...
    @Test void redirectInternal() throws Exception {
        var r=fetch(destinations.partnerUrl()+"/to-internal");assertEquals(200,r.statusCode());
        assertEquals(1,destinations.partnerHits.get());assertEquals(1,destinations.internalHits.get());
        assertEquals("INTERNAL-SYNTHETIC-REPORT",r.body());
        assertFalse(destinations.sawAuthorization.get());
    }
    @Test void sameOriginRedirect() throws Exception {
        var r=fetch(destinations.partnerUrl()+"/to-document");assertEquals(200,r.statusCode());
        assertEquals(2,destinations.partnerHits.get());assertEquals(0,destinations.internalHits.get());
    }
