...
@SpringBootTest(webEnvironment=SpringBootTest.WebEnvironment.RANDOM_PORT,
    properties="lab.csrf=${test.csrf:false}")
class BrowserReproductionTest extends BrowserHttp {
    @Test void missingToken() throws Exception {
        assertEquals(200, foreignPost("/api/approve", "invoice=C-1001").statusCode());
        approved(true);
    }
    @Test void invalidToken() throws Exception {
        assertEquals(200, foreignPost("/api/approve", "invoice=C-1001&_csrf=wrong").statusCode());
        approved(true);
    }
