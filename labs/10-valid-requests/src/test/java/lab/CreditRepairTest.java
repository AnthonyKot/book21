...
@SpringBootTest(webEnvironment=SpringBootTest.WebEnvironment.RANDOM_PORT,properties={"lab.gates=true","lab.conditional=${test.conditional:true}"})
class CreditRepairTest extends CreditHttp {
    ...
    @Test void aCommitsFirst()throws Exception{race("A",true,false);}
    @Test void bCommitsFirst()throws Exception{race("B",true,false);}
    @Test void bothReleased()throws Exception{
        ...
        System.out.println("RELEASE BOTH: "+statuses);assertEquals(List.of(200,409),statuses);state(1000,1);
    }
    @Test void distinctCredits()throws Exception{race("A",true,true);}
