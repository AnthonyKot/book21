...
The [local lab](https://github.com/AnthonyKot/book21/tree/main/labs/10-valid-requests) stores an `applied` flag in a credit row and a balance in an account row. Amounts are integer cents and tenant ownership is checked before applying a credit. This is a synthetic local fixture, not a payment service.
...
The [lab README](https://github.com/AnthonyKot/book21/blob/main/labs/10-valid-requests/README.md) gives the commands for both orders, first with the unconditional write and then with the repair. Predict the final balance as well as each response before running them. Reset only after the previous requests have finished.
...
Authentication, tenant checks and CSRF protection remain enabled in both modes. They answer different questions from whether two valid calls may perform the same effect.
...
Run both schedules with the conditional claim enabled. Now the first committed claimant receives `200`; the other receives `409`. The final balance is 1,000 and only one credit is applied. The guided tests also release both waiters without choosing a winner and require one success and one conflict.
...
The negative control removes the conditional predicate and runs the repair assertions again. Only the overlap cases fail; sequential replay, tenant checks and rollback still work. That result explains why an ordinary-request suite could look reassuring while missing the business-rule failure. A useful test must distinguish the guarantee it measures from the other guarantees that happen to hold.
...
These results belong to H2 2.4.240 at `READ_COMMITTED`, with multiple connections in one local application. Moving the statements to another database requires checking its isolation, affected-row behavior and failure/retry rules. Controlled schedules establish reproducible counterexamples and regressions; they do not enumerate every interleaving or establish crash recovery.

<!--mission-->

## Practice: choose the evidence for a second caller

The same application can apply credits through a worker entry point. Its contract is the same: each credit has at most one committed balance effect; an operation that fails before completion leaves neither write behind. Ordinary application, distinct credits and tenant rejection must continue to work.

Using the [worksheet](../practice/10-credit-worksheet.md), decide whether this path meets that contract. Choose the observations that would distinguish a duplicate effect, a partial effect and a correct application. Write your own tests before opening the supplied comparison check. Then make any justified change and show which test would expose its removal. A no-change conclusion also needs evidence and a deliberately broken counterexample.

Keep the result small: executable tests and a short explanation of the decision, one rejected alternative and the remaining limit. The worker is invoked through Spring; its HTTP adapter is synchronous and no queue is modeled. This is a changed caller within the mechanism you have just studied, not a search for an unknown vulnerability category. The [review guide](../practice/10-credit-review.md) provides progressive hints after a saved attempt.

The important question is where a request acquires the right to produce an effect, and what survives if it cannot finish. A final flag or a successful response answers neither question on its own.
