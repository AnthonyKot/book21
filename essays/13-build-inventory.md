...
The two application hashes match. The two image IDs differ. That is the concrete reason an application hash cannot stand in for an image identity. The [observed snapshot](https://github.com/AnthonyKot/book21/blob/main/labs/13-build-inventory/observed.json) summarizes this authoring run's counts and hashes; your rebuild produces its own image IDs, so use the values generated locally.

There are several different digests in this workflow. In the Docker setup measured here (Engine 29.1.3 with the classic overlay2 image store), the local image ID equals the SHA-256 of the image configuration, which references filesystem layers; other image stores can report a different object's digest, so record which store produced the ID. A registry manifest digest and a multi-platform index digest identify different objects. The base-image pin in our Dockerfile is a registry digest; the release-pairing exercise uses local image IDs obtained from Docker inspection. It does not claim these values are interchangeable. [OCI image identity](https://github.com/opencontainers/image-spec/blob/v1.1.1/config.md#imageid), [Docker manifest and platform distinctions](https://docs.docker.com/dhi/explore/security-concepts/digests/).
...
## Practice: which record describes the release?

The release manager has selected one image for today's release by its image ID. Five inventory records are attached to the change: from the release pipeline, the application build, a compliance export and a registry scanner. An advisory has just arrived about one version of the export helper, and someone has to answer whether the selected image contains it.

The [worksheet](../practice/13-inventory-worksheet.md) gives the packet in the lab's `data/release-review` directory. For each record, decide whether it describes the selected image, describes only part of it, describes something else, or cannot be relied on, and show the evidence you checked yourself rather than the record's own description. Then answer the advisory question, citing only records that can carry the answer, and say what evidence you would request before signing off.

Attempt it before opening the [review guide](../practice/13-inventory-review.md), which holds the verdicts, their reasons and a post-attempt check. No Docker or scanner is needed for this part; the guided run above is where those tools earn their place. The worksheet's time estimates are provisional. Every record states what it describes. The skill is noticing which of those statements the bytes support.
